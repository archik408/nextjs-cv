...
![Сравнение: одна команда специалистов вокруг одного продукта (Golden IT Age) и шесть изолированных Product Engineer с AI copilots, каждый со своим продуктом (AI IT Age)](/garden/end-of-programming/golden-vs-ai-it-age.webp)

## Вайб и нейрослоп

> Инженеры больше не нужны, и с лучшей подпиской на последнюю модель можно сделать всё что угодно
...
> ИИ же стремится к оверинжинирингу как к точке «совершенства» и перфекционизма
...
Сейчас всё немного изменилось: тесты тоже генерит агент, писать и чинить их стало дешевле. Если тест поломался — перегенерили. Кажется, что можно стремиться и к полному покрытию: модульные, интеграционные, системные. Плюс простой — имея такие gates, мы меньше боимся недетерминированности агентов. Оговорка конечно остаётся: дешёвые тесты это не значит полезные. Мусорный сгенерированный suite тоже нейрослоп, только с другой стороны.

Ещё один ложный успокаивающий жест — `/multi-model-review`: пусть одна модель (или субагент) ревьюит результат другой. Это не заменяет quality gates. Там тот же недетерминизм: модель проверяет модель, и на выходе снова вероятностный текст, а не формальная проверка. Циклы вроде **Ralph loop**, в которые так уходит кодогенерация, иногда просто неоправданны: агенты как машины пытаются перебрать все случаи и проблемные кейсы — даже когда это не нужно. Нет механизма компромисса. Эту роль раньше выполнял человек: он брал на себя ответственность за угловатости системы и её несовершенство. ИИ же **стремится к оверинжинирингу** как к точке «совершенства» и перфекционизма — и без внешней рамки может крутиться в этом цикле бесконечно.

## Оракул - путь к детерминизму?

> А ответственность — определённо на человеке. ИИ-агента никто не будет винить в том, что прод упал: он же машина.

Есть еще одно модное слово — это **«оракул»**. Говорят именно в этом кроется успех работы LLM. Но и тут тоже надо развеять туман определения, оракул — это что-то вроде главного агента, умного арбитра, который держит контекст всей задачи и направляет, ревьювает и калибрует работу быстрых агентов исполнителей. Они могут к нему обращаться за советом, это прагматичное разделение работы моделей. Оракул — мощная дорогая умная модель, исполнители — мелике и быстрые flash-модели. Но эта связка все еще алхимия. Видимо, это какая-то отсылка к древней греции: жрецы, оракулы, провидцы...

Для меня настоящий оракул это **исполняемая проверка**, которая без участия модели отвечает «да/нет» на конкретное ожидание. Спека в markdown — это постановка, все та же алхимия. Оракул — то, что _проверяет_ постановку. Если проверка не умеет кричать красным цветом — это не оракул, это пожелания и инструкции.

Мини-пример. Нужен логин. В чате агенту можно написать что угодно. Markdown-спека: «после верного пароля пользователь попадает в кабинет». Это все ещё алхимия. Оракул же — это тест вроде:
...
Агент может переписать хендлер на Express, Hapi, Hono, NestJS или свой роутер. Ему можно скормить другую модель, субагента, Ralph loop. Пока этот тест красный — успеха нет. Пока зелёный — поведение по этому контракту есть, даже если дифф выглядит инопланетно. Вот где живёт контекст успеха: не в истории чата, а в файле, который CI гоняет снова и снова.

У Bun «оракул» был жёстким и уже существовал к моменту миграции: тот же suite на совместимость с Node и на поведение рантайма. В [посте про Rust-порт](https://bun.com/blog/bun-in-rust) пишут прямо: механический порт при минимальных изменениях поведения, **тот же test suite**. Агенты писали `.rs` сколько угодно — мерило не «субагент Fable сказал ок», а «все тесты прошли».

Без этого миллион строк — это риск и лотерея.

Отсюда правило: сначала контракт, который умеет падать, потом генерация под него. Markdown `/plan` и OpenSpec полезны, чтобы _сформулировать_ ожидание и не потерять его между сессиями. Но пока из спеки не вырос хотя бы один падающий тест, инвариант типов или сканер с exit code — у вас постановка задачи, а не оракул.

Инструменты вторичны: они **воплощают** куски оракула. Жёсткое ядро — то, что детерминированно бьёт по логике: [мои нелюбимые типы](/garden/typescript) (`tsc`), unit/integration (Vitest/Jest). Вокруг — гигиена (ESLint/Oxlint, Prettier/Oxfmt), пока люди все ещё ходят читать код это нужно. E2e (Playwright) и Lighthouse — полезны, но часто шумные; нужно следовать по пирамиде тестирования или трофею, без фанатизма. Для массовых продуктов — проверка доступности с axe.

Для цепочки поставок — ясный контекст риска, тоже в виде проверок, а не абзаца в `AGENTS.md`: Semgrep по коду, Trivy и `npm audit` по зависимостям, Gitleaks по секретам, Syft как SBOM (inventory артефакта под CVE). CDN CSP/HSTS headers — оракул на краю рантайма. Имена можно менять, но инжененрный подход один: зафиксировать ожидание так, чтобы машина могла сказать «нет».

Именно такой набор **сужает класс ошибок** и делает приёмку бинарной: агент может галлюцинировать, но часть мусора отсечётся, пока линт, типы, тесты и сканеры не дадут exit code 0. Это практический harness — не markdown-заклинание, не оракул истины, а пайплайн.

Но есть ловушка. **Full access** для агента — опасная игрушка дьявола. Он без проблем залезет в каждый gate и отредактирует его: ослабит правило ESLint, выкинет «мешающий» тест, расширит `allowlist`, снимет порог Lighthouse. `ignore` и `allowlist` сами по себе чаще **ослабляют** проверки — ими агент как раз любит раздувать дыры. Нужна защита **конфигов gates**: CODEOWNERS / branch protection на CI, ESLint, пороги Lighthouse, lockfile и запрет агенту писать в эти файлы без человеческого ревью.

Пример простой. Агент без проблем поднимет версию пакета, если «так надо для сборки». Но вряд ли по умолчанию пойдёт смотреть CVE (`npm audit` / Trivy) и совместимость с платформами ваших пользователей. Варианта два: либо дать этот контекст в плане / спеке заранее, либо выстроить сильные gates, которые без разрешения того, кто берёт на себя ответственность, не пустят изменение дальше. А ответственность — определённо на человеке. ИИ-агента никто не будет винить в том, что прод упал: он же машина.
